#!/usr/bin/env node
// Verifies the committed nix.wasm is byte-identical to what the pinned
...
const pin = (await readFile(path.join(repoRoot, ".nix-grammar-pin"), "utf-8")).trim();
const committedBytes = await readFile(path.join(repoRoot, "nix.wasm"));
const committedHash = createHash("sha256").update(committedBytes).digest("hex");
console.log(`committed nix.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "nix-wasm-verify-"));
...
const rebuiltBytes = await readFile(path.join(work, "src", "tree-sitter-nix.wasm"));
const rebuiltHash = createHash("sha256").update(rebuiltBytes).digest("hex");
console.log(`rebuilt nix.wasm sha256: ${rebuiltHash}`);

if (committedHash !== rebuiltHash) {
    console.error("FAIL: committed nix.wasm does not match rebuild from pinned source");
    process.exit(1);
}
